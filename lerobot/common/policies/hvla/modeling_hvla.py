...
from lerobot.common.policies.hvla.paligemma_with_expert import (
...
class HVLAPolicy(PreTrainedPolicy):
    """Wrapper class around HVLAFlowMatching model to train and run inference within LeRobot."""
    ...
    def __init__(
        ...
        if config.bfm_latent_action:
            # BFM latents from the dataset are already on the unit-RMS sphere.
            # Skip Normalize/Unnormalize entirely (it'd be a no-op via the IDENTITY
            # entry in normalization_mapping, but using nn.Identity makes it
            # explicit and removes any chance of dataset-stats leakage).
        ...
        self.model = HVLAFlowMatching(config)
    ...
    def select_action(self, batch: dict[str, Tensor]) -> Tensor:
        ...
        if len(self._action_queue) == 0:
            ...
            self._action_queue.extend(actions.transpose(0, 1)[:20])
...
class HVLAFlowMatching(nn.Module):
    """
    π0: A Vision-Language-Action Flow Model for General Robot Control
    ...
    def sample_actions(
        ...
        **RTC（Real-Time Chunking）**：仅当 ``action_prefix`` 非空时启用——ΠGDM 伴随校正（论文 §3；PI 参考实现）。
        ...
        if self.config.bfm_latent_action:
            # Project the sampled chunk back onto the unit-RMS hypersphere
            # (per step). The flow-matching solver accumulates small magnitude
            # drift away from RMS=1, and the BFM decoder's behavior is invariant
            # to magnitude — but downstream callers / RTC prefix matching are
            # cleaner when every step is exactly on the sphere.
            rms = x_t.norm(dim=-1, keepdim=True) / (x_t.shape[-1] ** 0.5)
            x_t = x_t / (rms + 1e-8)
        return x_t
